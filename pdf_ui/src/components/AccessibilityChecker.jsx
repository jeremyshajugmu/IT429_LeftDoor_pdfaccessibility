...
const sanitizeForLog = (value) => {
  return String(value)
    ...
    .slice(0, 200);          // prevent overly large logs 
};
  ...
  const fetchBeforeReport = useCallback(async (retries = 3) => {
    ...
    for (let attempt = 1; attempt <= retries; attempt++) {
      try {
        ...
        console.log(
          `Attempt ${sanitizeForLog(attempt)}/${sanitizeForLog(retries)} failed for BEFORE report: ${sanitizeForLog(error.message)}`
        );
...
const fetchAfterReport = useCallback(async (retries = 3) => {
  ...
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      console.log(
        `Attempt ${sanitizeForLog(attempt)}/${sanitizeForLog(retries)} failed for AFTER report: ${sanitizeForLog(error.message)}`
      );
      if (attempt === retries) {
